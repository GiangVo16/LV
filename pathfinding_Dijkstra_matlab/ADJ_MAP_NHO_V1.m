...
inflate(OccMapFL, 28);           % Inflate map tránh va chạm
...
while true
    ...
    dis_map = occupancyMap(binMap, 100);
    dis_map_fl = copy(dis_map);
    inflate(dis_map_fl, 0.25);
    dis_map_fl.show
    xlabel('x [Met]');
    ylabel('y [Met]');
    grid on;
    ...
    a = 10;
    [path1, ~] = dijkstraWithTraceback(MAP, start, A, a);
    [path2, ~] = dijkstraWithTraceback(MAP, A, B, a);
    [path3, ~] = dijkstraWithTraceback(MAP, B, goal, a);
    ...
    converted_points = [x, y] / 100;  % Chuyển sang mét
